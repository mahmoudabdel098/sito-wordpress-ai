...
export default function Portfolio() {
  ...
  return (
    <div className="relative bg-black min-h-screen">
      ...
      <section ref={targetRef} className={`relative bg-black ${isMobile ? 'h-auto pb-40' : 'h-[800vh]'}`}>
        <div className={`relative pt-24 ${isMobile ? '' : 'sticky top-0 h-screen flex items-center pt-20 overflow-hidden'}`}>
          {isMobile ? (
            ...
            <motion.div 
              style={{ x }} 
              className="flex flex-row items-stretch gap-16 px-16"
            >
              {cardsContent}
            </motion.div>
          )}
      ...
      {!isMobile && (
        <div className="fixed bottom-6 sm:bottom-10 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-4 lg:gap-8 px-6 lg:px-10 py-3 lg:py-5 bg-black/80 backdrop-blur-2xl rounded-full border border-white/10 shadow-2xl w-auto lg:min-w-[500px]">
           ...
           <div className="flex items-baseline gap-1 min-w-[40px] justify-end">
              ...
              <span className="text-[8px] lg:text-[10px] font-bold text-white/40 leading-none">
